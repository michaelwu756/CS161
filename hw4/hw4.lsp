...
;;; returns *a clause* for the constraint not a or not b
;;; a and b are propositional variable indices
(defun not-both-true (a b)
  (list (* -1 a) (* -1 b)))

;;; returns *a list of clauses* for the constraint:
;;; "node n gets color c implies that node n does not get any color from the set {x,x+1,...,k}"
(defun at-most-one-color-helper (n c k x)
  (if (> x k)
      nil
    (cons (not-both-true (node2var n c k) (node2var n x k)) (at-most-one-color-helper n c k (+ x 1)))))
...
(defun at-most-one-color (n c k)
  (if (>= c k)
      nil
    (append (at-most-one-color-helper n c k (+ c 1)) (at-most-one-color n (+ c 1) k))))
...
;;; returns *a list of clauses* to ensure that
;;; "the nodes at both ends of edge e cannot have the same color from the set {c,c+1,...,k}"
(defun generate-edge-clauses-helper(e c k)
  (if (> c k)
      nil
    (cons (not-both-true (node2var (car e) c k) (node2var (car (cdr e)) c k))
